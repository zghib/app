...
export default {
  ...
  computed: {
    ...
    groupedFields() {
      const fieldsArray = Object.values(this.fields).filter(
        field => (this.permissions.read_field_blacklist || []).includes(field.field) === false
      );
      ...
      return result
        .filter(field => field.hidden_detail === false || field.hidden_detail == "0")
        .sort((a, b) => {
  ...
  methods: {
    ...
    deactivateField(field) {
      ...
      this.activeFields = this.activeFields.filter(activeField => activeField !== field);
      this.$emit("unstage-value", field);
    },
    isReadonly(field) {
      if (this.readonly) return true;
      if (field.readonly === true || field.readonly === "1" || field.readonly === 1) return true;
      if (this.permissions.write_field_blacklist.includes(field.field)) return true;
